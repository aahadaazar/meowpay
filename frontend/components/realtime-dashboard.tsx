...
export function RealtimeDashboard({ displayName, initialWallet, initialCats, initialEntries, initialRecipientCats }: RealtimeDashboardProps) {
  ...
  const [composerPrefill, setComposerPrefill] = useState<{ recipientWalletId: string; version: number } | null>(null);

  const onWalletChange = useCallback((changedWallet: WalletRealtimeRow) => {
    if (changedWallet.id === wallet.id) setWallet((current) => ({ ...current, balance: Number(changedWallet.balance) }));
    else setCats((current) => applyWalletChange(current, changedWallet));
  }, [wallet.id]);
  ...
  async function create(name: string) {
    ...
    try {
      ...
      setRecipientCats((current) => current.some((candidate) => candidate.walletId === cat.walletId) ? current : [...current, { walletId: cat.walletId, name: cat.name }]);
