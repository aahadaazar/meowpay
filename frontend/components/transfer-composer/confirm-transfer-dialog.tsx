...
export function ConfirmTransferDialog({
  ...
  async function confirm() {
    if (isConfirming || !transfer) return;
  ...
  return (
    ...
    >
      <section className="w-full max-w-[420px] rounded-lg border border-border bg-surface-card p-6 dark:bg-card">
        ...
        <h2 className="mt-2 text-title-lg" id="confirm-transfer-title">
          Send {transfer.amount.toLocaleString()} treats?
        </h2>
        <dl className="mt-6 grid gap-3 text-body-md">
          <div className="flex items-center justify-between gap-4">
            <dt className="text-body">From</dt>
            <dd className="font-semibold">{transfer.senderName}</dd>
          </div>
          <div className="flex items-center justify-between gap-4">
            <dt className="text-body">To</dt>
            <dd className="font-semibold">{transfer.receiverName}</dd>
          </div>
          {transfer.note ? (
            <div className="border-t border-border pt-3">
              <dt className="text-body">Note</dt>
              <dd className="mt-1">{transfer.note}</dd>
            </div>
          ) : null}
        </dl>
